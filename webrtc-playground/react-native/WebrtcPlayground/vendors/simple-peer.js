...
Peer.prototype.signal = function(data) {
  var self = this;
  if (self.destroyed) {
    throw makeError('cannot signal after peer is destroyed', 'ERR_SIGNALING');
  }
  ...
  if (data.sdp) {
    self._pc
      .setRemoteDescription(new self._wrtc.RTCSessionDescription(data))
      .then(function() {
        if (self.destroyed) {
          return;
        }
  ...
  if (
    ...
    self.destroy(
      makeError('signal() called with invalid signal data', 'ERR_SIGNALING'),
    );
  }
};
...
Peer.prototype._write = function(chunk, encoding, cb) {
  ...
  if (self.destroyed) {
    return cb(
      makeError('cannot write after peer is destroyed', 'ERR_DATA_CHANNEL'),
    );
  }
...
Peer.prototype._onIceStateChange = function() {
  ...
  if (iceConnectionState === 'closed') {
    self.destroy(
      makeError('Ice connection closed.', 'ERR_ICE_CONNECTION_CLOSED'),
    );
  }
};
    ...
    self.getStats(function(err, items) {
      ...
      function setSelectedCandidatePair(selectedCandidatePair) {
        ...
        if (self.remoteAddress) {
          self.remoteFamily = self.remoteAddress.includes(':')
            ? 'IPv6'
            : 'IPv4';
        }
...
Peer.prototype._onChannelOpen = function() {
  var self = this;
  if (self._connected || self.destroyed) {
    return;
  }
...
Peer.prototype._onTrack = function(event) {
  ...
  event.streams.forEach(function(eventStream) {
    ...
    if (
      self._remoteStreams.some(function(remoteStream) {
